...
func NewSetFromSlice(elems []interface{}) *Set {
	set := new(Set)
...
func NewSetFromIntSlice(elems []int) *Set {
	set := new(Set)
	set.mut = new(sync.Mutex)
	set.innerMap = make(map[interface{}]bool)
	set.results = make([]interface{}, 0)
...
func NewSetFromStringSlice(elems []string) *Set {
	set := new(Set)
	set.mut = new(sync.Mutex)
	set.innerMap = make(map[interface{}]bool)
	set.results = make([]interface{}, 0)
...
//IntElements return int elems
func (s *Set) IntElements() ([]int, error) {
	s.mut.Lock()
	defer s.mut.Unlock()
	return cast.ToIntSliceE(s.results)
}

//StringElements return string elems
func (s *Set) StringElements() ([]string, error) {
	s.mut.Lock()
	defer s.mut.Unlock()
	return cast.ToStringSliceE(s.results)
}
